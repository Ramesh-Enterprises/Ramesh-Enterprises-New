...
function updateBackground(){
  ...
  switch(val){
    case "night": document.body.style.background = "#0b1d2a"; break;
    case "snow": document.body.style.background = "#f2fafa"; break;
    default: document.body.style.background = ""; break;
  }
}
bgSelect.onchange = e => {savePref("bg", e.target.value); updateBackground();}
updateBackground();
...
function updateSnow(){
    ...
    for(let i=0;i<40;i++){
        ...
        s.style.top = Math.random()*-10 + "vh"; // start above view
...
snowEmoji.onchange = e => {savePref("snow", e.target.value); updateSnow();}

// WIND
function updateWind(){
    const styleEl = document.querySelector("#snowStyle");
    if(styleEl) styleEl.remove();
    const style = document.createElement("style");
    style.id = "snowStyle";
    let xMove = windDir.value == "left" ? "-" : windDir.value == "right" ? "+" : "0";
    style.innerHTML = `@keyframes fall {to{transform:translate(${xMove}20vw,110vh)}}`;
    document.head.appendChild(style);
}
windDir.onchange = e => {savePref("wind", e.target.value); updateWind();}
...
function playGame(name){
  recentPlay.textContent = "Recently Played: "+name;
  let f = JSON.parse(localStorage.getItem("favs")||"[]");
  if(!f.includes(name)){f.push(name); localStorage.setItem("favs",JSON.stringify(f))}
  renderFavs();
}
